...
class layerMenu
{
    ...
    public function layerMenu ( $themeLoc, $module, $debug = false)
    {
    ...
    {
        $head = "<SCRIPT src='" . XOOPS_URL . '/modules/' . $this->_module . "/js_files/JSCookMenu.js' type=text/javascript></SCRIPT>\n";
    ...
    public function buildMenu ($menuarray, $menuName)
    {
        $menu = '';
    ...
    // $menuType Supposrts the following options:

    // 'hbr' = Horizontal bar, dorps to bottom right;

    // 'hbl' = Horizontal bar, dorps to bottom left;

    // 'hur' = Horizontal bar, dorps to upper right;

    // 'hul' = Horizontal bar, dorps to upper left;

    // 'vbr' = Vertical bar, dorps to bottom right;

    // 'vbl' = Vertical bar, dorps to bottom left;

    // 'vur' = Vertical bar, dorps to upper right;

    // 'vul' = Vertical bar, dorps to upper left;

    public function getMenu($menuarray, $menuName, $treeTheme, $menuType = 'hbr')
    {
        $this->buildMenu($menuarray,$menuName);
        ...
        $draw = "\n<SCRIPT type=text/javascript><!-- \n cmDraw ('$menuName', $menuName, '$menuType', $treeTheme, '$treeTheme');\n --></SCRIPT>";
    ...
    public function doMain($mainitem,$child = FALSE,$t = "\t")
    {
        ...
        // check is this is not a menu item but rather a divider

        if (!is_array($mainitem)) {
            return $nline . $t . $mainitem;
        }
